...
import { demoCustomers, demoProducts, variantsByProduct } from "@/lib/mock";
import { formatBRL } from "@/lib/format";
import { EmptyState } from "@/components/empty-state";

/** Busca global sobre a base de demonstração (produtos e clientes). */
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="top-24 translate-y-0 gap-3 p-4">
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Busque produtos e clientes da loja
          </DialogDescription>
        </DialogHeader>
            ...
            {results.products.length > 0 && (
              <div>
                ...
                {results.products.map((product) => {
                  const stock = (variantsByProduct.get(product.id) ?? []).reduce(
                    (sum, v) => sum + v.stock,
                    0
                  );
                  return (
                    ...
                    >
                      ...
                      <span className="text-right">
                        ...
                        <span className="flex items-center justify-end gap-1 text-xs text-muted-foreground">
                          <Package className="size-3" /> {stock} un.
                        </span>
                      </span>
                    </button>
                  );
